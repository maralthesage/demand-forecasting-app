...
class EnsembleForecaster(BaseForecaster):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "EnsembleForecaster":
        ...
        """
        logger.info(
            f"Training ensemble model on {len(X)} samples with {len(X.columns)} features"
        )
        ...
        total_inverse_error = sum(
            1 / score for score in cv_scores.values() if score > 0
        )
...
class ProductForecaster:
    ...
    def fit(self, df: pd.DataFrame, feature_columns: List[str]):
            ...
            for category in df["product_category_id"].unique():
                ...
                if len(cat_data) >= self.min_history_months:
                    try:
                        ...
                        cat_model.fit(
                            cat_data[feature_columns], cat_data["anz_produkt"]
                        )
                        self.category_models[category] = cat_model
                        logger.info(f"Trained model for category {category}")
                    except Exception as e:
                        logger.error(
                            f"Error training category model for {category}: {str(e)}"
                        )
        ...
        for product_id in product_counts[
            product_counts >= self.min_history_months
        ].index:
            try:
                product_data = df[df["product_id"] == product_id]
                product_model = EnsembleForecaster()
                product_model.fit(
                    product_data[feature_columns], product_data["anz_produkt"]
                )
        ...
        logger.info(
            f"Trained {len(self.product_models)} product models, "
