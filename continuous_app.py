...
def load_current_data():
    ...
    try:
        ...
        if optimized_config.get("performance_metrics"):
            ...
            logger.info(
                "Using optimized model with %.1f%% improvement",
                metrics.get("improvement_percent", 0),
            )
        ...
        if not features_data.empty:
            ...
            search_index = {
                ...
                "stamm_products": set(
                    features_data["product_id"].str[:8].str.strip().unique()
                ),
...
def load_product_descriptions():
    ...
    try:
        ...
        if Path(desc_path).exists():
            ...
            desc_df = pd.read_csv(
                desc_path, usecols=["NUMMER", "BANAME1"], encoding="latin-1"
            )
...
def calculate_robust_trend(sales_data, horizon_months):
    ...
    linear_trend = [
        slope * (len(sales_data) + i) + intercept for i in range(1, horizon_months + 1)
    ]
    ...
    if len(smoothed) > 1:
        ...
        exp_trend = [
            smoothed[-1] + trend_slope * (i + 1) for i in range(horizon_months)
        ]
    else:
...
def calculate_seasonal_component(sales_data, horizon_months):
    ...
    for month in range(1, 13):
        if month in monthly_data and len(monthly_data[month]) > 0:
            ...
            seasonal_indices[month] = (
                month_mean / overall_mean if overall_mean > 0 else 1
            )
        else:
    ...
    if len(sales_data) >= 24:
        ...
        dominant_freq_idx = np.argmax(np.abs(fft_result[1:half_length])) + 1
        ...
        for i in range(horizon_months):
            ...
            seasonal_value = (
                0.1 * np.sin(phase) * np.mean(sales_data)
...
def advanced_forecast(product_data, forecaster, feature_columns, months=6):
    ...
    product_id = (
        product_data["product_id"].iloc[0]
...
def create_forecast_chart(
    ...
    forecast_values = advanced_forecast(
        original_product_data, forecaster, feature_columns or [], months=6
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            line=dict(
                color="#d62728", width=3, dash="dash", shape="spline", smoothing=0.3
            ),
...
def main():
    ...
    with col3:
        ...
        if st.button(
            "🔄 Refresh Models",
            help="Clear ALL cached data and reload with new forecasting algorithms",
        ):
            st.cache_data.clear()
            st.success(
                "🔄 **Cache Cleared!** Reloading with enhanced forecasting for ALL products..."
            )
            st.info(
                "This refreshes the entire app cache, not just the current product. "
                "All products will use the new forecasting algorithms."
            )
            st.rerun()
    ...
    if search_term:
        ...
        if len(search_upper) <= 8:
            ...
            matching_products = [
                pid
                for pid in search_index["product_lookup"]
    ...
    if search_term and len(search_term) >= 3 and not selected_product:
        ...
        suggestions = [
            pid
            for pid in search_index["product_lookup"]
            if pid.startswith(stamm_search)
        ][:5]
    ...
    if selected_product:
        # Get product data
        product_data = features_data[
            features_data["product_id"] == selected_product
        ].copy()
        ...
        if is_test_product:
            st.info(
                "🚀 **Enhanced Forecasting Active** - Using advanced trend and seasonality "
                "analysis for this test product"
            )
            ...
            with st.expander("🔍 Debug Information (Test Product)", expanded=False):
                ...
                date_start = product_data["MONAT"].min().strftime("%Y-%m")
                date_end = product_data["MONAT"].max().strftime("%Y-%m")
                st.write(f"**Date Range:** {date_start} to {date_end}")
                min_sales = product_data["anz_produkt"].min()
                max_sales = product_data["anz_produkt"].max()
                st.write(f"**Sales Range:** {min_sales:.0f} to {max_sales:.0f} units")
                recent_sales = product_data.tail(6)["anz_produkt"].tolist()
                st.write(f"**Last 6 months:** {recent_sales}")
                ...
                if len(duplicates) > 0:
                    st.error(
                        f"⚠️ **Data Issue Found:** {len(duplicates)} months have duplicate records!"
                    )
                    # Convert Timestamp keys to strings for JSON serialization
                    duplicates_dict = {
                        str(k): int(v) for k, v in duplicates.to_dict().items()
                    }
                ...
                if len(product_data) >= 12:
                    monthly_avg = product_data.groupby(product_data["MONAT"].dt.month)[
                        "anz_produkt"
                    ].mean()
        ...
        with col2:
            ...
            st.metric(
                "6-Month Total Sales Forecast", f"{int(total_forecast_sales):,} units"
            )
            avg_monthly_forecast = total_forecast_sales / 6
            st.metric(
                "Average Monthly Forecast",
                f"{int(avg_monthly_forecast):,} units",
            )

            # Compare with historical average
            historical_avg = product_data["anz_produkt"].mean()
            change_pct = (
                (avg_monthly_forecast - historical_avg) / historical_avg * 100
                if historical_avg > 0
                else 0
            )
        ...
        stamm = (
            selected_product[:8].strip()
            if len(selected_product) >= 8
        ...
        file_name = f"sales_forecast_{selected_product}_{timestamp}.csv"
