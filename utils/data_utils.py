...
def normalize_product_id(df: pd.DataFrame, product_col: str = 'product_id') -> pd.DataFrame:
    """
...
def detect_covid_period(date_col: pd.Series, 
                       covid_start: str = '2020-01-01', 
                       covid_end: str = '2022-12-31') -> pd.Series:
    """
...
def normalize_covid_sales(sales: pd.Series, 
                         is_covid: pd.Series, 
                         normalization_factor: float = 0.8) -> pd.Series:
    """
...
def detect_stock_outs(sales: pd.Series, 
                     stock_history: Optional[pd.Series] = None,
...
def detect_price_changes(price_series: pd.Series, 
...
def clean_categorical_data(df: pd.DataFrame, 
                          cat_columns: List[str]) -> pd.DataFrame:
    """
...
def aggregate_sales_data(df: pd.DataFrame, 
                        group_cols: List[str], 
                        date_col: str = 'MONAT',
                        sales_col: str = 'MENGE',
                        price_col: str = 'PREIS') -> pd.DataFrame:
    """
    ...
    result = result.rename(columns={
        sales_col: 'anz_produkt',
        price_col: 'unit_preis'
    })
...
def validate_data_quality(df: pd.DataFrame, 
    ...
    quality_report['missing_columns'] = [col for col in required_cols if col not in df.columns]
    ...
    date_cols = [col for col in df.columns if 'date' in col.lower() or 'monat' in col.lower()]
    if date_cols:
...
def filter_outliers(df: pd.DataFrame, 
    ...
    logger.info(f"Removed {outliers_removed} outliers from {column} using {method} method")
