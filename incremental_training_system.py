...
class IncrementalTrainingSystem:
    ...
    def initial_training(self, force_retrain: bool = False) -> Tuple[pd.DataFrame, ProductForecaster]:
        """
        ...
        try:
            forecaster = ProductForecaster(min_history_months=MODEL_CONFIG['min_history_months'])
        ...
        try:
            self.cache_training_results(integrated_data, features_data, forecaster, feature_columns)
        except Exception as e:
    ...
    def cache_training_results(self, integrated_data: pd.DataFrame, features_data: pd.DataFrame, 
                             forecaster: ProductForecaster, feature_columns: List[str]):
    ...
    def detect_new_data(self) -> Tuple[bool, pd.DataFrame]:
        ...
        try:
            ...
            # Extract new data
            new_data_mask = pd.Series(list(zip(current_data['product_id'], current_data['MONAT']))).isin(new_keys)
            new_data = current_data[new_data_mask].copy()
            
            logger.info(f"Detected {len(new_data)} new records for {new_data['product_id'].nunique()} products")
    ...
    def incremental_update(self) -> Tuple[pd.DataFrame, ProductForecaster]:
        ...
        try:
            ...
            updated_base_data = pd.concat([cached_base_data, new_data], ignore_index=True)
            updated_base_data = updated_base_data.drop_duplicates(subset=['product_id', 'MONAT'])
            ...
            # Combine features
            updated_features = pd.concat([features_data, new_features], ignore_index=True)
            updated_features = updated_features.drop_duplicates(subset=['product_id', 'MONAT'])
            feature_columns = self.feature_engineer.select_features(updated_features)
            
            # Incremental model update strategy
            new_products = set(new_data['product_id'].unique()) - set(features_data['product_id'].unique())
            ...
            if new_products:
                ...
                for product_id in new_products:
                    try:
                        product_data = updated_features[updated_features['product_id'] == product_id]
                        if len(product_data) >= MODEL_CONFIG['min_history_months']:
                            ...
                            if len(train_data) >= 3:  # Minimum for training
                                ...
                                product_model.fit(train_data[feature_columns], train_data['anz_produkt'])
                                forecaster.product_models[product_id] = product_model
                                logger.info(f"Trained model for new product {product_id}")
                    except Exception as e:
                        logger.error(f"Error training model for product {product_id}: {str(e)}")
            
            # Update cache with new data
            self.cache_training_results(updated_base_data, updated_features, forecaster, feature_columns)
    ...
    def quick_load_for_app(self) -> Tuple[pd.DataFrame, ProductForecaster, List[str]]:
        ...
        try:
            ...
            logger.info(f"Quick load completed: {len(features_data)} records, {len(feature_columns)} features")
